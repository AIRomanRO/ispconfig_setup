...
InstallSQLServer() {
  if [ $CFG_SQLSERVER == "MySQL" ]; then
    ...
    echo "mysql-server mysql-community-server/root_password password $CFG_MYSQL_ROOT_PWD" | debconf-set-selections
    echo "mysql-server mysql-community-server/root_password_again password $CFG_MYSQL_ROOT_PWD" | debconf-set-selections
