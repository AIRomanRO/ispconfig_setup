...
InstallWebServer() {
	...
	echo -n -e "$IDENTATION_LVL_1 Selected web Server: "
	
	if [ $CFG_WEBSERVER == "apache" ]; then
		echo -e "$IDENTATION_LVL_2 ${BBlack} Apache2 ${NC}"
	elif [ $CFG_WEBSERVER == "nginx" ]; then		
		if [ $CFG_NGINX_VERSION == "default" ]; then
			echo -e "$IDENTATION_LVL_2 ${BBlack} NGINX ${NC} "
	...
	if [ $CFG_WEBSERVER == "apache" ]; then
		...
		if [ $CFG_NGINX_VERSION == "default" ]; then
			...
			echo -n -e "$IDENTATION_LVL_2 ${BBlack} NGINX {NC} - Version: ${green} Custom - With OpenSSL 1.1 and ChaCha20-Poly1305 ${NC} \n"
