...
InstallWebServer() { 
	...
	if [ $CFG_WEBSERVER == "apache" ]; then
		...
		if [ $CFG_NGINX_VERSION == "n-os-default" ]; then
			...
			package_install nginx -t jessie/nginx
			echo -e "[ ${green}DONE${NC} ]"
			...
			package_install libssl1.0.0=1.0.2l-1~bpo8+1
			package_install nginx/jessie-nginx-http2 nginx-full/jessie-nginx-http2
			echo -e " [ ${green}DONE${NC} ]"
			...
			package_purge_remove -t stretch nginx
			echo -e " [ ${green}DONE${NC} ]"
