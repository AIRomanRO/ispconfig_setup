...
AskQuestions() {
    ...
    do
		CFG_SQLSERVER=$(whiptail --title "SQLSERVER" --backtitle "$WT_BACKTITLE" --nocancel --radiolist \
		    "Select SQL Server type" 10 50 3 \
		    "MySQL" "(default)" ON \
		    "MariaDB" "" OFF \
	...
	if [ $CFG_MYSQL_ROOT_PWD_AUTO == false ]; then
	    ...
	    do
	        CFG_MYSQL_ROOT_PWD=$(whiptail --title "MySQL" --backtitle "$WT_BACKTITLE" --inputbox \
				"Please specify a root password" --nocancel 10 50 3>&1 1>&2 2>&3)
	    done
	else
	  ...
	  if [ $CFG_WEBSERVER == "nginx" ]; then
		...
		do
			...
			"Select CertBot Version" 10 50 3 \
			"default" "from jessie backports" ON \				
			"stretch" "from stretch" OFF
			3>&1 1>&2 2>&3)
		done
	  ...
	  if [ $CFG_PHPMYADMIN == "yes" ]; then
	    ...
	    do
			...
			"From Where Do you want to install phpMyAdmin?" 10 50 3 \
