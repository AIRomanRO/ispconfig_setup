...
InstallPHPMyAdmin() {
  ...
  if [ $CFG_PHPMYADMIN == "yes" ]; then
    ...
    APP_DB_PASS=$(tr </dev/urandom -dc 'A-Z-a-z-0-9~!@#^*_=-' | head -c${1:-16})
    echo -e " [ ${green}DONE${NC} ] "
    ...
    # 	echo -e " [ ${green}DONE${NC} ] "
    elif [ $CFG_PHPMYADMIN_VERSION == "buster" ]; then
      echo -n -e "$IDENTATION_LVL_1 Install Buster Version ... "
      package_install phpmyadmin -t buster
      echo -e " [ ${green}DONE${NC} ] "
      ...
      # mkdir -p $PROGRAMS_INSTALL_DOWNLOAD/phpmyadmin
      # echo -e " [ ${green}DONE${NC} ] "

      echo -n -e "$IDENTATION_LVL_2 Download from PHPMyAdmin ... "
      wget -q -O "$PROGRAMS_INSTALL_DOWNLOAD/phpMyAdmin-latest.tar.gz" "https://files.phpmyadmin.net/phpMyAdmin/${PMA_VER}/phpMyAdmin-${PMA_VER}-all-languages.tar.gz"
      echo -e " [ ${green}DONE${NC} ] "

      echo -n -e "$IDENTATION_LVL_2 Extract the downloaded file ... "
      tar zxf $PROGRAMS_INSTALL_DOWNLOAD/phpMyAdmin-latest.tar.gz -C $PROGRAMS_INSTALL_DOWNLOAD/
      echo -e " [ ${green}DONE${NC} ] "

      # echo -n -e "$IDENTATION_LVL_2 Remove the downloaded file ... "
      # rm -rf $PROGRAMS_INSTALL_DOWNLOAD/phpmyadmin/phpMyAdmin-latest.tar.gz
      # echo -e " [ ${green}DONE${NC} ] "
      ...
      cp -Rpf $PROGRAMS_INSTALL_DOWNLOAD/phpMyAdmin-*/* /usr/share/phpmyadmin
      echo -e " [ ${green}DONE${NC} ] "
      ...
      echo "CREATE USER 'phpmyadmin'@'localhost' IDENTIFIED BY '$APP_DB_PASS';" >$PROGRAMS_INSTALL_SQLS/phpMyAdmin.sql
      echo "GRANT ALL PRIVILEGES ON phpmyadmin.* TO 'phpmyadmin'@'localhost';" >>$PROGRAMS_INSTALL_SQLS/phpMyAdmin.sql
      echo "FLUSH PRIVILEGES;" >>$PROGRAMS_INSTALL_SQLS/phpMyAdmin.sql

      mysql -u root -p$CFG_MYSQL_ROOT_PWD <$PROGRAMS_INSTALL_SQLS/phpMyAdmin.sql >>$PROGRAMS_INSTALL_LOG_FILES 2>&1
      #rm -f $PROGRAMS_INSTALL_SQLS/phpMyAdmin.sql
      echo -e " [ ${green}DONE${NC} ] "
      ...
      sed -i "$ a\$cfg['TempDir'] = '/var/lib/phpmyadmin/tmp';" /etc/phpmyadmin/config.inc.php
      echo -e " [ ${green}DONE${NC} ] "
      ...
      sed -i '/\*\sStorage\sdatabase\sand\stables\s\*/,/\*\*/ s|^//||' /etc/phpmyadmin/config.inc.php
      echo -e " [ ${green}DONE${NC} ] "
