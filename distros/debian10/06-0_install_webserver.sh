...
InstallWebServer() {
	...
	if [ $CFG_WEBSERVER == "apache" ]; then
		...
		elif [ $CFG_NGINX_VERSION == "n-buster" ]; then
			echo -e "${BBlack} NGINX ${NC} "
			echo -e "$IDENTATION_LVL_2 Version: ${green} Debian Buster Backports - with HTTP2 ${NC}"
	...
	if [ $CFG_WEBSERVER == "apache" ]; then
		...
		if [ $CFG_NGINX_VERSION == "n-os-default" ]; then
			...
			package_install nginx -t buster
			echo -e "[ ${green}DONE${NC} ]"


		elif [ $CFG_NGINX_VERSION == "n-buster" ]; then

			echo -e "$IDENTATION_LVL_2 ${BBlack}Version${NC}: ${green} Debian Buster${NC}"
			package_install -t buster-backports nginx
			echo -e " [ ${green}DONE${NC} ]"

		elif [ $CFG_NGINX_VERSION == "n-custom" ]; then
			echo -n -e "$IDENTATION_LVL_2 ${BBlack}Version: ${green} Custom - With OpenSSL 1.1 and ChaCha20-Poly1305 ${NC} \n"

			echo -n -e "$IDENTATION_LVL_2 Install the latest available from buster (in order to use the configurations form it) "
			package_install -t buster nginx
			echo -e " [ ${green}DONE${NC} ]"
			...
			package_install openssl libssl-dev -t buster
			echo -e " [ ${green}DONE${NC} ]"	
