InstallFix() {
  ...
  if [ $CFG_WEBMAIL == "roundcube" ] || [ $CFG_WEBMAIL == "roundcube-latest" ]; then
    ...
    if [ -f /etc/php/7.1/apache2/php.ini ]; then
      sed -i 's/;openssl.cafile=/openssl.cafile=\/etc\/ssl\/certs\/ca-certificates.crt/' /etc/php/7.1/apache2/php.ini
    fi

    if [ -f /etc/php/7.1/fpm/php.ini ]; then
      sed -i 's/;openssl.cafile=/openssl.cafile=\/etc\/ssl\/certs\/ca-certificates.crt/' /etc/php/7.1/fpm/php.ini
    fi


    if [ -f /etc/php/7.2/apache2/php.ini ]; then
      sed -i 's/;openssl.cafile=/openssl.cafile=\/etc\/ssl\/certs\/ca-certificates.crt/' /etc/php/7.2/apache2/php.ini
    fi

    if [ -f /etc/php/7.2/fpm/php.ini ]; then
      sed -i 's/;openssl.cafile=/openssl.cafile=\/etc\/ssl\/certs\/ca-certificates.crt/' /etc/php/7.2/fpm/php.ini
    fi
    ...
    if [ -f /etc/php/7.4/apache2/php.ini ]; then
      sed -i 's/;openssl.cafile=/openssl.cafile=\/etc\/ssl\/certs\/ca-certificates.crt/' /etc/php/7.4/apache2/php.ini
    fi
  ...
  if [$CFG_WEBSERVER == "nginx"]; then
    ...
    sed -i 's/error_page 497 https:\/\/\$host:$CFG_ISPONCFIG_APPS_PORT\$request_uri;/error_page 497 https:\/\/\$host:$CFG_ISPONCFIG_APPS_PORT\$request_uri;/' /etc/nginx/sites-available/apps.vhost >>$PROGRAMS_INSTALL_LOG_FILES 2>&1
    ...
    sed -i 's/ssl_protocols TLSv1 TLSv1.1 TLSv1.2;/ssl_protocols TLSv1 TLSv1.1 TLSv1.2 TLSv1.3;/' /etc/nginx/sites-available/apps.vhost >>$PROGRAMS_INSTALL_LOG_FILES 2>&1
