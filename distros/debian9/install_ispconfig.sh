...
InstallISPConfig() {
	...
	wget -q -O $PROGRAMS_INSTALL_DOWNLOAD/ISPConfig-3-stable-latest.tar.gz https://ispconfig.org/downloads/ISPConfig-3.1.14p2.tar.gz >> $PROGRAMS_INSTALL_LOG_FILES 2>&1
	...
	if [ $CFG_ISPC == "standard" ]; then
		...
		if getTrueFalseFormat $CFG_SETUP_WEB; then
	      echo "configure_jailkit=" getYNFormat $CFG_JKIT  >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
	    else
	      echo "configure_jailkit=n" >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
	    fi

	    echo "configure_ftp=" getYNFormat $CFG_SETUP_WEB >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
		echo "configure_dns=" getYNFormat $CFG_SETUP_NS >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
	    echo "configure_apache=" getYNFormat $CFG_APACHE >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
		echo "configure_nginx=" getYNFormat $CFG_NGINX >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
		...
		echo "do_backup=y" >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
		echo "mysql_root_password=$CFG_MYSQL_ROOT_PWD" >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
	    echo "mysql_master_hostname=$CFG_MASTER_FQDN" >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
		echo "mysql_master_root_user=root" >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
	    echo "mysql_master_root_password=$CFG_MASTER_MYSQL_ROOT_PWD" >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
		echo "mysql_master_database=dbispconfig" >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
		echo "reconfigure_permissions_in_master_database=n" >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
		echo "reconfigure_services=y" >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
		echo "apps_vhost_port=$CFG_ISPONCFIG_APPS_PORT" >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
	    echo "ispconfig_port=$CFG_ISPONCFIG_PORT" >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
		echo "create_new_ispconfig_ssl_cert=n" >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
	    echo "reconfigure_crontab=y" >> $AUTO_INSTALL_INI_LOCATION/autoinstall.ini
